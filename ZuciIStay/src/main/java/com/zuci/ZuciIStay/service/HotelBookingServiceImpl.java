...
public class HotelBookingServiceImpl implements HotelBookingService
{
    ...
    @Override
    public boolean roomBookingByCheck(BookingData bookingData,String username) {
        ...
        Booking book=new Booking(0,username,bookingData.getPlace(),bookingData.getHotelName(),bookingData.getRoomType(),bookingData.getFromDate(),bookingData.getToDate());
        return true;
    }

    @Override
    public List<Booking> getBookingByUsername(String username) {
        Optional<Booking> optional=hotelBookingRepository.findByUsername(username);
        if(optional.isPresent()){
            List<Booking> bookList = optional
